...
export async function extractTemplate(framework, targetDir, variant) {
  ...
  // 首先尝试使用非压缩方式，直接复制目录
  const directoryPath = path.resolve(templatesDir, framework, variant);
  console.log(`尝试从目录复制模板: ${directoryPath}`);
  
  try {
    if (await fs.pathExists(directoryPath)) {
      console.log(`发现直接目录模板，使用复制方式`);
      await fs.copy(directoryPath, targetDir, { overwrite: true });
      console.log(`成功复制${framework}模板（${variant}）`);
      return;
    }
  } catch (dirError) {
    console.error(`目录复制模式出错: ${dirError.message}，尝试压缩包模式`);
  }
