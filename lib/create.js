...
async function createConfigFile(targetDir, language, options) {
  ...
  try {
    // 直接使用当前可执行文件所在目录下的templates目录
    const templatesDir = process.env.PACKAGE_ROOT || ".";
    ...
    if (await fs.pathExists(templateConfigPath)) {
      ...
      // 解压模板
      const archiveFile = path.resolve(templatesDir, "templates", `${framework}.tar.gz`);
      if (await fs.pathExists(archiveFile)) {
        ...
        if (result) {
          ...
          throw new Error(`无法从模板压缩包提取配置文件`);
        }
      } else {
