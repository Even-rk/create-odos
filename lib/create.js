...
export async function createProject(options) {
  ...
  // 获取模板目录
  const frameworkTemplateDir = path.join(templateDir, framework);

  // 检查模板目录是否存在
  if (!fs.existsSync(frameworkTemplateDir)) {
    throw new Error(`模板目录不存在: ${frameworkTemplateDir}`);
  }

  // 复制基础模板
  await copyBaseTemplate(frameworkTemplateDir, targetDir, language);
...
async function copyBaseTemplate(templateDir, targetDir, language) {
  const baseDir = path.join(templateDir, language);

  // 检查语言模板是否存在
  if (!fs.existsSync(baseDir)) {
    throw new Error(`语言模板不存在: ${baseDir}`);
  }

  // 不直接复制整个目录，而是逐个文件进行处理
  // 这样我们在后面可以根据用户选择过滤掉某些文件
...
async function createConfigFile(targetDir, language, options) {
  ...
  const templateConfigPath = path.join(templateDir, "build", configFilename);

  // 确保模板配置文件存在
  if (await fs.pathExists(templateConfigPath)) {
    await fs.copyFile(templateConfigPath, targetConfigPath);
  } else {
    throw new Error(`模板配置文件不存在: ${templateConfigPath}`);
  }
}
